...
func (c OnePhaseCommitProtocol[Ent, ID]) manager() testcase.Var[frameless.OnePhaseCommitProtocol] {
	return testcase.Var[frameless.OnePhaseCommitProtocol]{
		ID: "commit protocol manager",
		Init: func(t *testcase.T) frameless.OnePhaseCommitProtocol {
			return c.subject().Get(t).CommitManager
		},
	}
}

func (c OnePhaseCommitProtocol[Ent, ID]) resource() testcase.Var[CRD[Ent, ID]] {
...
func (c OnePhaseCommitProtocol[Ent, ID]) Spec(s *testcase.Spec) {
	...
	s.Describe(`OnePhaseCommitProtocol`, func(s *testcase.Spec) {
		...
		s.Test(`BeginTx+CommitTx, Creator/Reader/Deleter methods yields error on Context with finished tx`, func(t *testcase.T) {
			...
			if updater, ok := c.resource().Get(t).(frameless.Updater[Ent]); ok {
		...
		s.Test(`BeginTx should be callable multiple times to ensure emulate multi level transaction`, func(t *testcase.T) {
			...
			t.Must.Nil(c.manager().Get(t).CommitTx(tx2InTx1), `"inner" tx should be considered done`)
			t.Must.Nil(c.manager().Get(t).CommitTx(tx1), `"outer" tx should be considered done`)
...
func (c OnePhaseCommitProtocol[Ent, ID]) specPurger(s *testcase.Spec) {
	...
	s.Test(`entity created prior to transaction won't be affected by a purge after a rollback`, func(t *testcase.T) {
		...
		tx, err := c.manager().Get(t).BeginTx(ctxVar.Get(t))
		t.Must.Nil(err)

		t.Must.Nil(purger(t).Purge(tx))
		IsAbsent[Ent, ID](t, c.resource().Get(t), ctxVar.Get(t), HasID[Ent, ID](t, ptr))
		IsFindable[Ent, ID](t, c.resource().Get(t), ctxVar.Get(t), HasID[Ent, ID](t, ptr))
...
func (c OnePhaseCommitProtocol[Ent, ID]) specCreatorPublisher(s *testcase.Spec) {
	publisher := func(t *testcase.T) CreatorPublisherSubject[Ent, ID] {
		p, ok := c.resource().Get(t).(CreatorPublisherSubject[Ent, ID])
		if !ok {
	...
	s.Describe(`.Subscribe/Create`, func(s *testcase.Spec) {
		subscriber := letSubscriber[Ent, ID](s, func(event interface{}) bool {
			_, ok := event.(frameless.CreateEvent[Ent])
			return ok
		})
		...
		events := testcase.Let(s, func(t *testcase.T) []*Ent {
			return genEntities[Ent](t, c.MakeEnt)
		})
		subject := func(t *testcase.T) (frameless.Subscription, error) {
			return publisher(t).SubscribeToCreatorEvents(ctxVar.Get(t), subscriber.Get(t))
		}
		...
		s.Before(func(t *testcase.T) {
			...
			for _, entity := range events.Get(t) {
				Create[Ent, ID](t, c.resource().Get(t), ctxVar.Get(t), entity)
			}
		})
		...
		s.Then(`before a commit, events will be absent`, func(t *testcase.T) {
			...
			t.Must.Nil(c.manager().Get(t).CommitTx(ctxVar.Get(t)))
		})

		s.Then(`after a commit, events will be present`, func(t *testcase.T) {
			t.Must.Nil(c.manager().Get(t).CommitTx(ctxVar.Get(t)))

			var es []frameless.CreateEvent[Ent]
			for _, ent := range events.Get(t) {
				es = append(es, frameless.CreateEvent[Ent]{Entity: *ent})
			}
		...
		s.Then(`after a rollback, events will be absent`, func(t *testcase.T) {
			t.Must.Nil(c.manager().Get(t).RollbackTx(ctxVar.Get(t)))
			Waiter.Wait()
			t.Must.Empty(subscriber.Get(t).Events())
		})
	})
...
func (c OnePhaseCommitProtocol[Ent, ID]) specUpdaterPublisher(s *testcase.Spec) {
	...
	updaterPublisher := func(t *testcase.T) UpdaterPublisherSubject[Ent, ID] {
		u, ok := c.resource().Get(t).(UpdaterPublisherSubject[Ent, ID])
		if !ok {
	...
	s.Describe(`.Subscribe/Update`, func(s *testcase.Spec) {
		subscriber := letSubscriber[Ent, ID](s, func(event interface{}) bool {
			_, ok := event.(frameless.UpdateEvent[Ent])
			return ok
		})
		ctxVar.Let(s, func(t *testcase.T) context.Context {
			t.Log(`given we are in transaction`)
		...
		subject := func(t *testcase.T) (frameless.Subscription, error) {
			return updaterPublisher(t).SubscribeToUpdaterEvents(ctxVar.Get(t), subscriber.Get(t))
		}
		onSuccess := func(t *testcase.T) frameless.Subscription {
			sub, err := subject(t)
			t.Must.Nil(err)
		...
		s.Before(func(t *testcase.T) {
			...
			for _, ptr := range events.Get(t) {
				Update[Ent, ID](t, updater(t), ctxVar.Get(t), ptr)
			}
		})
		...
		s.Then(`after a commit, events will be present`, func(t *testcase.T) {
			...
			var es []frameless.UpdateEvent[Ent]
			for _, ent := range events.Get(t) {
				es = append(es, frameless.UpdateEvent[Ent]{Entity: *ent})
			}
...
func (c OnePhaseCommitProtocol[Ent, ID]) specDeleterPublisher(s *testcase.Spec) {
	publisher := func(t *testcase.T) DeleterPublisherSubject[Ent, ID] {
		u, ok := c.resource().Get(t).(DeleterPublisherSubject[Ent, ID])
		if !ok {
	...
	s.Describe(`#SubscribeToDeleteByID`, func(s *testcase.Spec) {
		subscriber := letSubscriber[Ent, ID](s, func(event interface{}) bool {
			_, ok := event.(frameless.DeleteByIDEvent[ID])
			return ok
		})
		ctxVar.Let(s, func(t *testcase.T) context.Context {
			t.Log(`given we are in transaction`)
		...
		entity := testcase.Let(s, func(t *testcase.T) *Ent {
			return toPtr(c.MakeEnt(t))
		})
		subject := func(t *testcase.T) (frameless.Subscription, error) {
			return publisher(t).SubscribeToDeleterEvents(ctxVar.Get(t), subscriber.Get(t))
		}
		onSuccess := func(t *testcase.T) frameless.Subscription {
			sub, err := subject(t)
			t.Must.Nil(err)
		...
		hasDeleteEntity := DeleterPublisher[Ent, ID]{}.hasDeleteEntity
		subscription := letSubscription[Ent, ID](s)
		...
		s.Before(func(t *testcase.T) {
			...
			Create[Ent, ID](t, c.resource().Get(t), ctxVar.Get(t), entity.Get(t))
		...
		s.Then(`after a commit, delete events will arrive to the subscriber`, func(t *testcase.T) {
			...
			hasDeleteEntity(t, subscriber.Get(t).Events, frameless.DeleteByIDEvent[ID]{ID: HasID[Ent, ID](t, entity.Get(t))})
		})
	...
	s.Describe(`#SubscribeToDeleteAll`, func(s *testcase.Spec) {
		...
		s.Before(func(t *testcase.T) {
			...
			DeleteAll[Ent, ID](t, c.resource().Get(t), ctxVar.Get(t))
			Waiter.Wait()
		})
		...
		s.Then(`after a commit, delete all event will arrive to the subscriber`, func(t *testcase.T) {
			...
			Eventually.Assert(t, func(tb assert.It) {
				...
				tb.Must.Contain(subscriber.Get(t).Events(), frameless.DeleteAllEvent{})
			})
		})
