...
type Config struct {
	...
	// HTTPRoundTripperFactory allows the injection of custom authentication solutions to .
	HTTPRoundTripperFactory httpkit.RoundTripperFactoryFunc
}
...
func (r Repository[ENT, ID]) Create(ctx context.Context, ptr *ENT) error {
	...
	if err != nil {
		return errorkit.F("failed to map %T to its dto format", ptr)
	}

	data, err := keyToDTO(dto)
	if err != nil {
		return err
	}
...
func (r Repository[ENT, ID]) FindByID(ctx context.Context, id ID) (ENT, bool, error) {
	...
	response, err := r.Client.Secrets.KvV2Read(ctx, r.getVaultPath(id), requestOptions...)
	if err != nil {
		var zero ENT
		var vaultResponseError *vault.ResponseError
		if errors.As(err, &vaultResponseError) {
			if vaultResponseError.Errors == nil && vaultResponseError.StatusCode == 404 {
				// valid use case with nothing to return
	...
	ptrDTO := r.Mapper.NewDTO()
	if err := dtoToKey(response.Data.Data, ptrDTO); err != nil {
		var zero ENT
		return zero, false, err
	}

	ent, err := r.Mapper.MapFromDTO(ctx, ptrDTO)
	if err != nil {
		var zero ENT
		return zero, false, err
	}
...
func keyToDTO(key any) (vaultDTOFormat, error) {
	data, err := json.Marshal(key)
	if err != nil {
		return nil, err
	}
	var vaultDataDTO vaultDTOFormat
	err = json.Unmarshal(data, &vaultDataDTO)
	return vaultDataDTO, err
}
