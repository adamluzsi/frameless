...
type Client struct {
	// BaseURL is the Vault server's base address in the form of scheme://host:port.
	...
	// HTTPRoundTripperFactory, when supplied, decorates the http.RoundTripper used
	// by the underlying HTTP client. This makes it possible to inject middleware into
	// the request pipeline, such as a Kubernetes auth round tripper that attaches a
	// freshly issued Vault token to each outgoing request.
	//
	// The factory receives the next (downstream) http.RoundTripper and is expected to
	// return a http.RoundTripper that eventually delegates to it.
	HTTPRoundTripperFactory func(next http.RoundTripper) http.RoundTripper

	// HTTPClient is the HTTP client used for the direct read operations (e.g. FindByID).
	// It is the very same client the underlying vault Client uses, so that the configured
	// http.RoundTripper pipeline is applied consistently across every operation.
	HTTPClient *http.Client

	// GetToken, when set, returns the Vault token used to authenticate the requests.
	// It is called on each request, which allows dynamic / short-lived tokens
	// (for example issued by a Kubernetes auth integration) to be injected
...
var DefaultRequestTimeout time.Duration = 30 * time.Second

func (c *Client) httpClient() (*http.Client, error) {
	// http client initialized through the vault client init
...
func (c *Client) vaultClient() (*vault.Client, error) {
	return synckit.InitErr[*vault.Client](&c.rwm, &c.vclient, func() (*vault.Client, error) {
		var opts []vault.ClientOption
		if c.HTTPClient != nil {
			opts = append(opts, vault.WithHTTPClient(c.HTTPClient))
		}
...
func (c *Client) HealthCheck(ctx context.Context) health.Report {
	...
	vaultClient, err := c.vaultClient()
	if err != nil {
		return report.WithIssue(ctx, health.Issue{
			Code:    "vault-client-init-error",
			Message: err.Error(),
			Causes:  health.Unknown,
		})
	}

	resp, err := vaultClient.System.ReadHealthStatus(ctx, opts...)
...
func (r Repository[ENT, ID]) FindAll(ctx context.Context) iter.Seq2[ENT, error] {
	...
	return iterkit.From(func(yield func(ENT) bool) error {
		...
		response, err := vaultClient.Secrets.KvV2List(ctx, r.basePath(), requestOptions...)
		if err != nil {
...
func (r Repository[ENT, ID]) FindByID(ctx context.Context, id ID) (ENT, bool, error) {
	...
	// Build the Vault KV v2 read endpoint URL: /v1/{mount}/data/{path}.
	// The mount and the secret path are each escaped as a single path segment, mirroring
	// how the HashiCorp vault client builds the write request path (see KvV2Write). This
	// keeps IDs that contain '/' (already percent-escaped by getVaultPath) pointing at the
	// exact same Vault location on write and on read.
	mountPath := trimSlash(r.MountPoint)
	if mountPath == "" {
		mountPath = "kv-v2"
	}
	vaultPath := r.getVaultPath(id)
	requestPath := "/v1/" + url.PathEscape(mountPath) + "/data/" + url.PathEscape(vaultPath)
	endpointURL := strings.TrimRight(r.Client.BaseURL, "/") + requestPath
	...
	httpClient, err := r.Client.httpClient()
	if err != nil {
		var zero ENT
		return zero, false, fmt.Errorf("failed to initialize http client: %w", err)
	}
	...
	// On write every number was stored as a JSON string to survive Vault's lossy
	// float64 parsing; convert them back into numbers guided by the DTO type before
	// unmarshaling, so the typed fields receive their exact original values.
	dataData, err := jsonStringsToNumbers(kvResp.Data.Data, reflect.TypeOf(dtoPtr))
	if err != nil {
		var zero ENT
		return zero, false, fmt.Errorf("error normalizing kv v2 data.data for %T: %w", dtoPtr, err)
	}

	if err := json.Unmarshal(dataData, dtoPtr); err != nil {
		var zero ENT
...
func (r Repository[ENT, ID]) DeleteByID(ctx context.Context, id ID) error {
	...
	_, err = vaultClient.Secrets.KvV2Delete(ctx, vaultRecordPath, requestOptions...)
	return err
}
...
func (r Repository[ENT, ID]) PermanentDeleteByID(ctx context.Context, id ID) error {
	...
	if _, err := vaultClient.Secrets.KvV2ReadMetadata(ctx, vaultRecordPath, requestOptions...); err != nil {
		var vaultResponseError *vault.ResponseError
		if errors.As(err, &vaultResponseError) {
	...
	_, err = vaultClient.Secrets.KvV2DeleteMetadataAndAllVersions(ctx, vaultRecordPath, requestOptions...)
	return err
}
...
func toVaultDataDTO(key any) (vaultDTOFormat, error) {
	...
	// Vault decodes JSON numbers into float64 values server-side, silently losing
	// precision for integers that don't fit into float64's 53-bit mantissa. Store every
	// number as a JSON string so Vault keeps it verbatim; FindByID restores the original
	// numeric types based on the DTO definition.
	data, err = jsonNumbersToStrings(data)
	if err != nil {
		return nil, err
	}
...
func (ki KubernetesIntegration) CreateToken(ctx context.Context) (*JWTLoginData, error) {
	...
	vaultClient, err := ki.Client.vaultClient()
	if err != nil {
		return nil, err
	}
	...
	resp, err := vaultClient.Auth.JwtLogin(ctx, req, vault.WithMountPath(ki.MountPoint))
	if err != nil {
