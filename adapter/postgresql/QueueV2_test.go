...
func TestQueueV2(t *testing.T) {
	s := testcase.NewSpec(t)

	var (
		makeQueue = let.Var(s, func(t *testcase.T) func(queueV2Config) queueV2Subject {
			return func(cfg queueV2Config) queueV2Subject {
				return postgresql.QueueV2[Entity]{
					Name: cfg.Name, Connection: cfg.Connection,
		...
		subject = let.Var(s, func(t *testcase.T) queueV2Subject {
			return makeQueue.Get(t)(config.Get(t))
		})
		ready = let.Var(s, func(t *testcase.T) queueV2Subject {
			q := subject.Get(t)
			assert.Must(t).NoError(q.Migrate(queueV2Context(t)))
			return q
		})
		other = let.Var(s, func(t *testcase.T) queueV2Subject {
		...
		deliverySub = let.Var(s, func(t *testcase.T) *queueV2Subscription {
			q := ready.Get(t)
			assert.Must(t).NoError(q.Publish(queueV2Context(t), data.Get(t)))
	...
	s.Describe("blocking #Publish", func(s *testcase.Spec) {
		act := func(t *testcase.T) <-chan error {
			...
			assert.Must(t).NoError(publisher.Migrate(queueV2Context(t)))
			value := data.Get(t)
			...
			go func() {
				defer close(done)
				result <- publisher.Publish(ctx, value)
			}()
			t.Cleanup(func() {
				cancel()
				select {
				...
				case <-time.After(queueV2OperationBudget):
					t.Error("blocking publisher did not stop after cancellation")
				}
			})
			return result
		}
	...
	s.Describe("concurrent processing", func(s *testcase.Spec) {
		...
		s.When("more handlers than pool connections are holding unsettled deliveries", func(s *testcase.Spec) {
			...
			s.Then("delivers all available messages and leaves the pool usable before any handler finishes", func(t *testcase.T) {
				messages := act(t)
				assert.True(t, len(messages) > int(poolSize.Get(t)))
	...
	s.Describe("simultaneous claims from independent pools", func(s *testcase.Spec) {
		...
		act := func(t *testcase.T) []pubsub.Message[Entity] {
			...
			for range batch.Get(t) {
				...
				assert.Must(t).NoError(cfg.Connection.DB.Ping(queueV2Context(t)))
			...
			for range consumers {
				select {
				case result := <-results:
					assert.Must(t).NoError(result.err)
		...
		s.Before(func(t *testcase.T) {
			for _, value := range batch.Get(t) {
				assert.Must(t).NoError(ready.Get(t).Publish(queueV2Context(t), value))
			}
		})
	...
	s.Describe("idle subscriptions", func(s *testcase.Spec) {
		...
		s.Test("more idle subscribers than connections release the pool and make progress after publishing", func(t *testcase.T) {
			...
			results := act(t)
			assert.True(t, len(batch.Get(t)) > int(poolSize.Get(t)))
			assert.Eventually(t, queueV2OperationBudget, func(it testing.TB) {
				stats := pool.Stat()
	...
	s.Describe("Message#ACK", func(s *testcase.Spec) {
		...
		s.When("the delivery was already acknowledged", func(s *testcase.Spec) {
			s.Before(func(t *testcase.T) { assert.Must(t).NoError(delivery.Get(t).ACK()) })
		...
		s.When("the delivery was released and a replacement owns the message", func(s *testcase.Spec) {
			s.Before(func(t *testcase.T) {
				assert.Must(t).NoError(delivery.Get(t).NACK())
				replacement.Get(t)
			})
	...
	s.Describe("settlement response loss at the adapter boundary", func(s *testcase.Spec) {
		...
		config.Let(s, func(t *testcase.T) queueV2Config {
			...
			injected, calls := responseLost.Get(t), settlementCalls.Get(t)
			// Only this queue's adapter is wrapped. Native claims and renewals, and
			// the healthy observer, bypass it; no private SQL matching is needed.
			cfg.Connection.DBAdapter = func(pool *pgxpool.Pool) flsql.Queryable {
				...
				return flsql.QueryableAdapter{
					...
					QueryRowFunc: func(ctx context.Context, query string, args ...any) flsql.Row {
						calls.Add(1)
						return queueV2LostResponseRow{Row: q.QueryRowContext(ctx, query, args...), Err: injected}
					},
				}
			}
			return cfg
	...
	s.Describe("decode failure", func(s *testcase.Spec) {
		...
		ownershipDuration.LetValue(s, 500*time.Millisecond)
		s.Before(func(t *testcase.T) {
			assert.Must(t).NoError(ready.Get(t).Publish(queueV2Context(t), data.Get(t)))
		})
	...
	s.Describe("#Migrate", func(s *testcase.Spec) {
		...
		s.Test("creates storage for a named queue", func(t *testcase.T) {
			assert.NoError(t, act(t))
			assert.NoError(t, subject.Get(t).Publish(queueV2Context(t), data.Get(t)))
		...
		s.When("another named queue has an active delivery and a waiting message", func(s *testcase.Spec) {
			s.Before(func(t *testcase.T) {
				q := other.Get(t)
				assert.Must(t).NoError(q.Publish(queueV2Context(t), batch.Get(t)[0]))
				assert.Must(t).NoError(q.Publish(queueV2Context(t), batch.Get(t)[1]))
			})
		...
		s.Test("creates distinct per-name message and receipt tables regardless of blocking mode", func(t *testcase.T) {
			assert.NoError(t, act(t))
			other.Get(t)
			assert.ContainsExactly(t, queueV2Tables(t, connection.Get(t)), []string{
				"frameless_qv2_messages_" + name.Get(t),
				"frameless_qv2_messages_" + name.Get(t) + "_other",
				"frameless_qv2_receipts_" + name.Get(t),
				"frameless_qv2_receipts_" + name.Get(t) + "_other",
			})
		})
		...
		s.When("blocking publishing is enabled", func(s *testcase.Spec) {
			...
			s.Then("creates the named receipt table alongside the message table", func(t *testcase.T) {
				assert.NoError(t, act(t))
				assert.NoError(t, act(t))
				assert.ContainsExactly(t, queueV2Tables(t, connection.Get(t)), []string{
					"frameless_qv2_messages_" + name.Get(t),
		...
		for _, validName := range []string{"a", "a0_b9", strings.Repeat("z", 40)} {
			s.When("Name is the supported boundary "+validName, func(s *testcase.Spec) {
				...
				s.Then("uses the full untruncated name in both backing tables", func(t *testcase.T) {
					assert.NoError(t, act(t))
					assert.ContainsExactly(t, queueV2Tables(t, connection.Get(t)), []string{
						"frameless_qv2_messages_" + name.Get(t),
						"frameless_qv2_receipts_" + name.Get(t),
	...
	for _, operation := range []struct {
		...
		s.Describe(operation.name+" name validation", func(s *testcase.Spec) {
			...
			for _, invalidName := range []string{
				"", "Messages", "0messages", "_messages", "two-queues", "two.queues", "two queues",
	...
	s.Describe("#Purge", func(s *testcase.Spec) {
		...
		s.When("another named queue has an active delivery and a waiting message", func(s *testcase.Spec) {
			s.Before(func(t *testcase.T) {
				assert.Must(t).NoError(other.Get(t).Publish(queueV2Context(t), batch.Get(t)[0]))
				assert.Must(t).NoError(other.Get(t).Publish(queueV2Context(t), batch.Get(t)[1]))
			})
		...
		s.When("this queue has an active lease as well as a waiting message", func(s *testcase.Spec) {
			...
			s.Before(func(t *testcase.T) {
				active.Get(t)
				assert.Must(t).NoError(ready.Get(t).Publish(queueV2Context(t), batch.Get(t)[0]))
			})
...
func queueV2Tables(tb testing.TB, conn postgresql.Connection) []string {
	...
	err := conn.QueryRowContext(queueV2Context(tb), `
		...
		WHERE n.nspname = current_schema() AND c.relkind IN ('r', 'p')
		  AND c.relname LIKE 'frameless_qv2_%'
	`).Scan(&names)
	assert.Must(tb).NoError(err)
	return names
}
...
func queueV2MessageCount(tb testing.TB, conn postgresql.Connection, name string) int {
	tb.Helper()
	var count int
	table := pgx.Identifier{"frameless_qv2_messages_" + name}.Sanitize()
