...
Migrate with `Blocking: true` before using a blocking publisher. This enables the optional receipt table and its expiry index; purely nonblocking queues do not create them. Enabling blocking publishing later and migrating again adds receipt storage without recreating the message table. Consumers do not need `Blocking: true`: ACK and purge detect and update receipt storage when it exists.
...
The configured name is the message table name and the prefix for auxiliary storage:

- `<name>`: `id TEXT`, `data BYTEA`, nullable `meta JSONB`, `created_at TIMESTAMPTZ`, nullable `owner TEXT` and `owned_until TIMESTAMPTZ` (both null for an unclaimed message).
- `<name>_receipts`: optional blocking-publisher observations, created only when migrating with `Blocking: true`.

The message ordering index is `<name>_order`; the optional receipt expiry index is `<name>_exp`.

Names fit PostgreSQL's standard 63-byte identifier limit without truncation. Configure all pool connections with the same stable `search_path`/database namespace. Same names in the same namespace share storage. Queue names must not collide with another queue's generated table or index names. Separate tables do not imply independent database capacity.

Migration is repeatable and preserves messages. Once created, receipt storage is retained even when migrating through a nonblocking instance, since other instances may still have waiting publishers. Existing receipt tables are not automatically removed. Same-name concurrent migrations are serialized only during schema creation. Changing a configured value's `Name` selects another queue; it does not rename or move existing storage. Already-created subscriptions/deliveries keep their original configuration. Do not mutate configuration concurrently with operations.
