...
func (m *Cache[ENT, ID]) InvalidateByID(ctx context.Context, id ID) (rErr error) {
	...
	HitsThatReferenceOurEntity := iterkit.Filter[Hit[ID]](hitsIter, func(h Hit[ID]) bool {
	...
	hitIDs, err := iterkit.Reduce(HitsThatReferenceOurEntity, []HitID{},
...
func (m *Cache[ENT, ID]) CachedQueryMany(ctx context.Context, hitID HitID, query QueryManyFunc[ENT]) (_ iter.Seq[ENT], _ iterkit.ErrFunc, rErr error) {
	// TODO: double check
	if ctx != nil {
	...
	if found {
		...
		iter, e, err := m.Repository.Entities().FindByIDs(ctx, hit.EntityIDs...)
...
func (m *Cache[ENT, ID]) CachedQueryOne(
	...
	iter, e, err := m.CachedQueryMany(ctx, hitID, m.mapQueryOneToQueryMany(query))
	if err != nil {
		return _ent, false, err
	}
	ent, found := iterkit.First[ENT](iter)
	if err := e(); err != nil {
		return _ent, false, err
	}
	if !found {
...
func (m *Cache[ENT, ID]) Refresh(ctx context.Context) (rErr error) {
	...
	errorsIter := iterkit.Map(allEntitiesIter, func(v ENT) (error, error) {
	...
	errs, err := iterkit.Collect(errorsIter)
	if err != nil {
		return err
	}
...
func (m *Cache[ENT, ID]) mapQueryOneToQueryMany(q QueryOneFunc[ENT]) QueryManyFunc[ENT] {
	return func(ctx context.Context) (iter.Seq[ENT], func() error, error) {
		...
		if !found {
			return iterkit.Empty[ENT](), nil
		}
		return iterkit.SingleValue(ent), nil
	}
}
