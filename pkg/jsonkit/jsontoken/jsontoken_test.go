...
func TestQuery(t *testing.T) {
	...
	s.Test("smoke", func(t *testcase.T) {
		...
		got, err := iterkit.Collect(jsontoken.Query(ctx, bytes.NewReader(data), jsontoken.KindArray, jsontoken.KindArrayValue))
		assert.NoError(t, err)
...
func TestArrayIterator(t *testing.T) {
	...
	s.Test("smoke", func(t *testcase.T) {
		...
		got2, err := iterkit.Collect(jsontoken.Query(context.Background(), bytes.NewReader(data),
