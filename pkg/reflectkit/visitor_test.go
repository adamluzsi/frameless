...
func TestVisitor(t *testing.T) {
	...
	visitor := let.Var(s, func(t *testcase.T) reflectkit.Visitor {
		return reflectkit.Visitor{}
	})
	...
	s.Describe("#Iter", func(s *testcase.Spec) {
		...
		act := let.Act(func(t *testcase.T) iter.Seq[reflectkit.V] {
			return visitor.Get(t).Iter(value.Get(t))
		})

		s.Context("on struct value", func(s *testcase.Spec) {
			type T struct {
				A int
				B int
			}
		...
		s.Context("V#Is", func(s *testcase.Spec) {
			...
			s.Test("nested pointer elem", func(t *testcase.T) {
				...
				vvs := iterkit.Collect(visitor.Get(t).Iter(reflect.ValueOf(v)))
		...
		s.Context("smoke", func(s *testcase.Spec) {
			s.Test("visited struct field can be set", func(t *testcase.T) {
				...
				var vX reflectkit.V
				for v := range visitor.Get(t).Iter(rv) {
					if v.NodeType != refnode.StructField {
						continue
					}
					if v.StructField.Name == "X" {
			...
			s.Test("setting a field value using the visited reflection value", func(t *testcase.T) {
				...
				for v := range visitor.Get(t).Iter(reflect.ValueOf(&foo)) {
					if v.NodeType != refnode.StructField {
						continue
					}
					if v.StructField.Name == "Foo" {
