...
func TestClone(t *testing.T) {
	...
	s.When("the value is a time.Time that carries a location", func(s *testcase.Spec) {
		value.Let(s, func(t *testcase.T) reflect.Value {
			// a time in UTC keeps a nil *time.Location, which hides the
			// problem, so the location has to be an actual one here.
			return reflect.ValueOf(t.Random.Time().In(time.Local))
		})

		s.Then("the location is aliased instead of being deep copied", func(t *testcase.T) {
			src := value.Get(t).Interface().(time.Time)

			got, ok := act(t).Interface().(time.Time)
			assert.True(t, ok)
			assert.True(t, got.Equal(src))

			// *time.Location values are process wide immutable singletons
			// (time.UTC, time.Local, the time.LoadLocation cache), and the time
			// package identifies a zone by the pointer itself. Deep copying one
			// detaches the clone from the original zone, duplicates the whole
			// zone table on every clone, and reads fields that the time package
			// only populates behind its own lazy initialisation.
			assert.True(t, got.Location() == src.Location(),
				assert.MessageF("expected the clone to share the origin's *time.Location, but got a copy of it"))
		})

		s.Then("the clone stays equal to its origin", func(t *testcase.T) {
			src := value.Get(t).Interface().(time.Time)

			got, ok := act(t).Interface().(time.Time)
			assert.True(t, ok)

			// time.Time is a comparable type, and it is regularly compared with
			// == through map keys, struct equality and assert.Equal on a value
			// that embeds it.
			assert.True(t, src == got,
				assert.MessageF("expected the cloned time.Time to remain == to its origin\nsrc: %#v\ngot: %#v", src, got))
		})
	})
}
