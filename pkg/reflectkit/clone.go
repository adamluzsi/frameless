...
func CloneT[T any](v T) T {
	var out = Clone(reflect.ValueOf(v))
	if !out.IsValid() {
		var zero T
		return zero
	}
...
type _CloneConfig struct {
	noCLoneRegister map[reflect.Type]struct{}
	mutex           sync.RWMutex
}

func (r *_CloneConfig) Register(typ reflect.Type) {
	r.mutex.Lock()
	defer r.mutex.Unlock()
	if r.noCLoneRegister == nil {
		r.noCLoneRegister = make(map[reflect.Type]struct{})
	}
	r.noCLoneRegister[typ] = struct{}{}
}

func (r *_CloneConfig) CanClone(v reflect.Value) bool {
	r.mutex.RLock()
	defer r.mutex.RUnlock()
	if !v.IsValid() {
		return false
	}
	_, ok := r.noCLoneRegister[v.Type()]
	return !ok
}

var cloneConfig _CloneConfig

// DoNotClone will flag a type for non-clone able.
// For example, Singleton value types such as *time.Location,
// can be registered as a type which must not be cloned.
func DoNotClone[T any]() struct{} {
	cloneConfig.Register(TypeOf[T]())
	return struct{}{}
}

var _ = DoNotClone[*time.Location]()
