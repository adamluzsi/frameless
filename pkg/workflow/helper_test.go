...
type C struct {
	...
	ProcessSignalQueue testcase.Var[*memory.Queue[workflow.ProcessID]]
...
func letC(s *testcase.Spec) C {
	...
	c.ProcessSignalQueue = let.Var(s, func(t *testcase.T) *memory.Queue[workflow.ProcessID] {
		return &memory.Queue[workflow.ProcessID]{}
	})
