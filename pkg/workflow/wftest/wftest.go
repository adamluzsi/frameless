...
type C struct {
	...
	Scheduler                   testcase.Var[*workflow.Scheduler]
	SchedulerRunErr             testcase.Var[error]
	ProcessRepository           testcase.Var[*memory.Repository[workflow.Process, workflow.ProcessID]]
	EventsRepository            testcase.Var[*memory.WorkflowEventRepository]
	ProcessQueue                testcase.Var[*memory.Queue[workflow.ProcessScheduleEntry]]
	ProcessQueueChangeBroadcast testcase.Var[*memory.Queue[workflow.ProcessQueueChange]]
}
...
func LetC(s *testcase.Spec) C {
	...
	c.ProcessQueue = let.Var(s, func(t *testcase.T) *memory.Queue[workflow.ProcessScheduleEntry] {
		return &memory.Queue[workflow.ProcessScheduleEntry]{
			SortLessFunc: func(i, j workflow.ProcessScheduleEntry) bool {
	...
	c.EventsRepository = let.Var(s, func(t *testcase.T) *memory.WorkflowEventRepository {
		return &memory.WorkflowEventRepository{}
	})

	c.ProcessQueueChangeBroadcast = let.Var(s, func(t *testcase.T) *memory.Queue[workflow.ProcessQueueChange] {
		return &memory.Queue[workflow.ProcessQueueChange]{}
	})

	c.SchedulerRunErr = let.VarOf[error](s, nil)

	c.Scheduler = let.Var(s, func(t *testcase.T) *workflow.Scheduler {
		var sch = &workflow.Scheduler{
			Runtime:                     pointer.Of(c.Runtime.Get(t)),
			ProcessQueue:                c.ProcessQueue.Get(t),
			ProcessRepository:           c.ProcessRepository.Get(t),
			ProcessQueueChangeBroadcast: c.ProcessQueueChangeBroadcast.Get(t),
		}
		go func() {
	...
	c.Runtime = let.Var(s, func(t *testcase.T) workflow.Runtime {
		return workflow.Runtime{
			...
			EventsRepository: c.EventsRepository.Get(t),
		}
	})
