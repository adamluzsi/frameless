	...
	s.Describe("#Execute", func(s *testcase.Spec) {
		...
		s.When("it has an element", func(s *testcase.Spec) {
			var (
				pid   = wftest.LetParticipantID(s)
				count = let.VarOf(s, 0)
				part  = wftest.LetParticipant(s, c, pid, func(t *testcase.T) func(ctx context.Context) error {
					return func(ctx context.Context) error {
						count.Set(t, count.Get(t)+1)
						return nil
					}
		...
		s.When("it has multiple elements", func(s *testcase.Spec) {
			...
			wftest.LetParticipant(s, c, fooPid, func(t *testcase.T) func(context.Context) error {
				return func(ctx context.Context) error {
					testcase.Append(t, callOrder, fooPid.Get(t))
					return nil
				}
			})
			mid := wftest.LetParticipant(s, c, barPid, func(t *testcase.T) func(context.Context) error {
				return func(ctx context.Context) error {
					testcase.Append(t, callOrder, barPid.Get(t))
					return nil
				}
			})
			wftest.LetParticipant(s, c, bazPid, func(t *testcase.T) func(context.Context) error {
				return func(ctx context.Context) error {
...
func TestIsCompleted(t *testing.T) {
	...
	s.When("EventCompleted event is present", func(s *testcase.Spec) {
		process.Let(s, func(t *testcase.T) *workflow.Process {
			p := process.Super(t)
			p.Events = workflow.NewEvents(append(mustHistory(t, p), &workflow.EventCompleted{})...)
			return p
		})
		...
		s.And("other events as well present in the event history", func(s *testcase.Spec) {
			process.Let(s, func(t *testcase.T) *workflow.Process {
				...
				p.Events = workflow.NewEvents(append(events, mustHistory(t, p)...)...)
				return p
			})
...
func TestSetVar(t *testing.T) {
	...
	s.Describe("#Execute", func(s *testcase.Spec) {
		var (
			...
			process = wftest.LetProcessWithDefinition(s, subject)
		)
		act := let.Act(func(t *testcase.T) error {
		...
		s.Then("execution is idempotent with runtime", func(t *testcase.T) {
			...
			firstPassEvents := mustHistory(t, process.Get(t))

			t.Random.Repeat(3, 7, func() {
				assert.NoError(t, act(t))

				assert.Equal(t, mustHistory(t, process.Get(t)), firstPassEvents)
			})
		})
	})
}
