...
func TestProcess_json_smoke(tt *testing.T) {
	...
	assert.NotEmpty(tt, mustHistory(tt, &p))
...
func TestDefinition_json_smoke(tt *testing.T) {
	...
	s.Test("Sequence", func(t *testcase.T) {
		...
		assert.NoError(t, err)
		assert.NotNil(t, decoded.V)
		assert.Equal[workflow.Definition](t, pdef, decoded.V)
	})

	s.Test("If with condition and participants", func(t *testcase.T) {
		pdef := workflow.If{
		...
		assert.NotNil(t, decoded.V)
		assert.Equal[workflow.Definition](t, decoded.V, pdef)
	})
	...
	s.Test("SetVar as Definition", func(t *testcase.T) {
		pdef := workflow.SetVar{
			Key:   workflow.VariableKey(t.Random.String()),
			Value: t.Random.String(),
		}

		data, err := json.Marshal(pdef)
		assert.NoError(t, err)
		t.Logf("Encoded: %s", string(data))
