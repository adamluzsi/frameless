...
func (m Mux) ServeCLI(w Response, r *Request) {
	...
	if !ok {
		...
		m.helpUsage(w)
		m.helpLineBreak(w, 1)
		m.printErrLn(w, "command is unknown: "+name, "")
		m.helpCommands(w)
		return
	}
	...
	if err := m.serveCLI(entry, name, w, r); err != nil {
		w.ExitCode(ExitCodeBadRequest)
		m.helpUsageOf(w, entry, name)
		m.helpLineBreak(w, 1)
		m.printErrLn(w, err.Error())
	}
}
...
func (m Mux) serveCLI(e *muxEntry, name string, w Response, r *Request) error {
	...
	if e.meta != nil {
		...
		var flagSet = flag.NewFlagSet(name, flag.ContinueOnError)
		flagSet.Usage = func() {}
		flagSet.SetOutput(&flagSetOutput)

		var callbacks []func() error

		for _, f := range e.meta.Flags {
			f := f // scope variable
		...
		for _, a := range e.meta.Args {
			raw, ok := slicekit.Lookup(r.Args, a.Index)
			if !ok && a.Required {
				return ErrArgParseIssue.F("%s argument is not provided", a.Name)
			}
			if err := a.Setter(val, raw); err != nil {
				return err
			}
		}
...
func (m Mux) helpLineBreak(w Response, n int) {
	for i := 0; i < n; i++ {
		m.errOut(w).Write([]byte(lineSeparator))
	}
}
...
func (m Mux) helpUsageOf(w Response, e *muxEntry, name string) {
	var lines []string

	var usage string
	usage += "Usage: " + m.getPath() + " " + name

	if e.meta != nil {
		if 0 < len(e.meta.Flags) {
			usage += " [OPTION]..."
		}
		if 0 < len(e.meta.Args) {
	...
	if s, ok := e.Handler.(HelpSummary); ok {
...
func (m Mux) printErrLn(w Response, msg ...string) {
	if len(msg) == 0 {
		return
	}
	m.errOut(w).Write([]byte(strings.Join(msg, lineSeparator) + lineSeparator))
}

func (m Mux) errOut(w Response) io.Writer {
...
func scanForArg(sf reflect.StructField) (structArg, bool, error) {
	...
	return structArg{
		...
		Desc:     desc,
		Required: isRequired,
		Enum:     enum.ReflectValues(sf.Type),
	}, true, nil
}
...
func (sa structArg) Setter(Struct reflect.Value, raw string) (rErr error) {
	defer errorkit.Recover(&rErr)
	field := Struct.FieldByIndex(sa.StructField.Index)
	if len(raw) == 0 && sa.HasDefault {
		field.Set(sa.DefVal)
		return nil
	}
	rval, err := convkit.ParseReflect(field.Type(), raw)
