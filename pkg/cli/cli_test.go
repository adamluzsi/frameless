...
func TestMux(t *testing.T) {
	...
	s.Describe("#ServeCLI", func(s *testcase.Spec) {
		...
		request.Let(s, func(t *testcase.T) *cli.Request {
			...
			r := request.Super(t)
			slicekit.Unshift(&r.Args, commandName.Get(t))
			return r
		})
		...
		s.And("the command have flags", func(s *testcase.Spec) {
			...
			s.And("the command supports args", func(s *testcase.Spec) {
				var h = testcase.LetValue[*CommandWithArg](s, nil)

				command.Let(s, func(t *testcase.T) cli.Handler {
					return CommandWithArg{Callback: func(v CommandWithArg, w cli.Response, r *cli.Request) {
						h.Set(t, &v)
				...
				s.And("the required flags are provided", func(s *testcase.Spec) {
					request.Let(s, func(t *testcase.T) *cli.Request {
						r := request.Super(t)
						r.Args = append(r.Args, "string-val-0", "42", "true")
						return r
					})

					s.Then("flags are parsed and injected as dependency to the handler", func(t *testcase.T) {
						act(t)

						assert.Contain(t, response.Get(t).Out.String(), "tea")

						assert.NotNil(t, h.Get(t))
						assert.NotEmpty(t, h.Get(t).Arg0)
						assert.NotEmpty(t, h.Get(t).Arg1)
						assert.NotEmpty(t, h.Get(t).Arg2)
						assert.NotEmpty(t, h.Get(t).Arg3)

						assert.Equal(t, h.Get(t).Arg0, "string-val-0")
						assert.Equal(t, h.Get(t).Arg1, 42)
						assert.Equal(t, h.Get(t).Arg2, true)
						assert.Equal(t, h.Get(t).Arg3, "default-value")
					})
				})
			})
		})

		s.And("the command have ARGS", func(s *testcase.Spec) {
			var h = testcase.LetValue[*CommandWithFlag](s, nil)
...
func (c AndTheFlagTypeIs[T]) Spec(s *testcase.Spec) {
	...
	s.And("one of the flag is a "+name+" type"+c.Desc, func(s *testcase.Spec) {
		s.Before(func(t *testcase.T) {
			t.OnFail(func() {
				t.Log("code:", c.Response.Get(t).Code)
				t.Log("args:", c.Request.Get(t).Args)
				t.Log("\nout:\n", c.Response.Get(t).Out.String())
				t.Log("\nerr:\n", c.Response.Get(t).Err.String())
			})
		})
...
func (c AndTheArgTypeIs[T]) Spec(s *testcase.Spec) {
	var name = reflectkit.SymbolicName(reflectkit.TypeOf[T]())
	s.And("one of the ARG is a "+name+" type"+c.Desc, func(s *testcase.Spec) {
		s.Before(func(t *testcase.T) {
			t.OnFail(func() {
				t.Log("code:", c.Response.Get(t).Code)
				t.Log("args:", c.Request.Get(t).Args)
...
type CommandWithArgWDefault struct {
	Callback[CommandWithArgWDefault]
	Arg string `arg:"0" default:"defval"`
}

func (cmd CommandWithArgWDefault) ServeCLI(w cli.Response, r *cli.Request) {
	cmd.Callback.Call(cmd, w, r)
}
