...
// MatchPrefix reports whether the slice match another slice as its prefix.
//
// An empty prefix matches every path.
func MatchPrefix[S, P ~[]T, T any](s S, prefix P) bool {
	if len(s) < len(prefix) {
		return false
	}
	for i := range len(prefix) {
		if !comp.Equal[T](s[i], prefix[i]) {
			return false
		}
	}
	return true
