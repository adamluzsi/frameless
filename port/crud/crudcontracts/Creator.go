...
func Creator[ENT, ID any](subject crud.Creator[ENT], opts ...Option[ENT, ID]) contract.Contract {
	...
	act := func(t *testcase.T) error {
		...
		if err == nil {
			...
			if ByIDFinderOK {
				c.Helper().IsPresent(t, fID, ctx, id)
			}
		}
		return err
	}
	...
	s.When(`entity was not saved before`, func(s *testcase.Spec) {
		...
		if ByIDFinderOK {
			s.Then(`after creation, the freshly made entity can be retrieved by its id`, func(t *testcase.T) {
				t.Must.NoError(act(t))
				t.Must.Equal(ptr.Get(t), c.Helper().IsPresent(t, fID, c.MakeContext(t), getID(t)))
			})
		}
	})
	...
	if c.SupportIDReuse {
		s.When(`entity ID is provided ahead of time`, func(s *testcase.Spec) {
			s.Before(func(t *testcase.T) {
				...
				if ByIDFinderOK {
					c.Helper().IsPresent(t, fID, c.MakeContext(t), getID(t))
				} else {
					wait()
				}
				...
				if ByIDFinderOK {
					c.Helper().IsAbsent(t, fID, c.MakeContext(t), getID(t))
				} else {
					wait()
				}
			})
	...
	if ByIDFinderOK {
		s.Test(`created entity can be retrieved with #FindByID`, func(t *testcase.T) {
			...
			t.Must.Equal(e, *c.Helper().IsPresent(t, fID, c.MakeContext(t), id))
		})
	}
