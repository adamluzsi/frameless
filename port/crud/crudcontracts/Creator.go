...
func Creator[ENT, ID any](subject crud.Creator[ENT], opts ...Option[ENT, ID]) contract.Contract {
	...
	d, byIDDeleterOK := subject.(crud.ByIDDeleter[ID])
	fID, ByIDFinderOK := subject.(crud.ByIDFinder[ENT, ID])
	// fAll, AllFinderOK := subject.(crud.AllFinder[ENT])
	...
	act := func(t *testcase.T) error {
		...
		if err == nil {
			...
			if ByIDFinderOK {
				crudtest.IsPresent[ENT, ID](t, fID, ctx, id)
			}
		}
		return err
	}
	...
	s.When(`entity was not saved before`, func(s *testcase.Spec) {
		...
		if ByIDFinderOK {
			s.Then(`after creation, the freshly made entity can be retrieved by its id`, func(t *testcase.T) {
				t.Must.NoError(act(t))
				t.Must.Equal(ptr.Get(t), crudtest.IsPresent[ENT, ID](t, fID, c.MakeContext(t), getID(t)))
			})
		}
	})
	...
	if c.SupportIDReuse {
		s.When(`entity ID is provided ahead of time`, func(s *testcase.Spec) {
			s.Before(func(t *testcase.T) {
				...
				if ByIDFinderOK {
					crudtest.IsPresent[ENT, ID](t, fID, c.MakeContext(t), getID(t))
				} else {
					wait()
				}
				...
				if ByIDFinderOK {
					crudtest.IsAbsent[ENT, ID](t, fID, c.MakeContext(t), getID(t))
				} else {
					wait()
				}
			})
	...
	if ByIDFinderOK {
		s.Test(`created entity can be retrieved with #FindByID`, func(t *testcase.T) {
			...
			if byIDDeleterOK {
				t.Defer(d.DeleteByID, c.MakeContext(t), id)
			}

			t.Must.Equal(e, *crudtest.IsPresent[ENT, ID](t, fID, c.MakeContext(t), id))
		})
	}
	...
	// 		})
	//
	// 		t.Must.Equal(e, *IsPresent[ENT, ID](t, fID, c.MakeContext(t), id))
	// 	})
	// }
