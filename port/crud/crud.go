...
type ByIDsFinder[ENT, ID any] interface {
	...
	FindByIDs(ctx context.Context, ids ...ID) (_ iter.Seq[ENT], release func() error, _ error)
}
...
type AllFinder[ENT any] interface {
	...
	FindAll(context.Context) (_ iter.Seq[ENT], release func() error, _ error)
}
...
type QueryManyMethodSignature[ENT, ARGS any] func(context.Context, ARGS) (_ iter.Seq[ENT], release func() error, _ error)

func Collect[T any](i iter.Seq[T], release func() error, err error) (_ []T, rErr error) {
	if err != nil {
		return nil, err
	}
	if release != nil {
		defer func() { rErr = release() }()
	}
	var vs = make([]T, 0)
	for v := range i {
		vs = append(vs, v)
