...
const typeNamePubSub = "PubSub"

func (ps *PubSub[Entity]) Publish(ctx context.Context, vs ...Entity) (rErr error) {
	var (
		keys      []string
		namespace = getNamespaceFor[Entity](typeNamePubSub, &ps.Namespace)
	)
...
func (ps *PubSub[Entity]) Purge(ctx context.Context) error {
	var namespace = getNamespaceFor[Entity](typeNamePubSub, &ps.Namespace)
...
fetch:
	...
	namespace := getNamespaceFor[Entity](typeNamePubSub, &pss.pubsub.Namespace)
...
func (pm *pubsubMessage[Entity]) ACK() error {
	...
	_, ok := pm.pubsub.Memory.lookup(getNamespaceFor[Entity](typeNamePubSub, &pm.pubsub.Namespace), pm.record.key)
	if !ok {
		return nil
	}
	pm.pubsub.Memory.Del(pm.ctx, getNamespaceFor[Entity](typeNamePubSub, &pm.pubsub.Namespace), pm.record.key)
	return nil
}
